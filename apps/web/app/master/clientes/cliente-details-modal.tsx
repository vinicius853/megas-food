        ...
        {tenant ? (
          <>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <DetailItem label="CPF/CNPJ" value={formatCpfCnpj(tenant.document)} />
              ...
              <div className="rounded-2xl border border-orange-100 bg-orange-50 p-4 sm:col-span-2">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-600">
                      Valor contratado:{' '}
                      {getCurrentSubscription(tenant)
                        ? formatMoney(getCurrentSubscription(tenant)?.contractedMonthlyPrice)
                        : '-'}
                    </p>
                  </div>
                  {canManagePlans ? (
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <DetailItem
                    ...
                    value={
                      getCurrentSubscription(tenant)?.plan
                        ? formatMoney(getCurrentSubscription(tenant)?.plan?.monthlyPrice)
                        : '-'
                    }
                  />
                  <DetailItem
                    label="Status da assinatura"
                    value={getCurrentSubscription(tenant)?.status || 'Sem assinatura'}
                  />
                  <DetailItem
                    label="Contratado em"
                    value={formatFullDate(getCurrentSubscription(tenant)?.contractedAt)}
                  />
                  <DetailItem
                    label="Proximo vencimento"
                    value={formatFullDate(getCurrentSubscription(tenant)?.nextBillingDate)}
                  />
                  <DetailItem
                    label="Acesso ate"
                    value={formatFullDate(getCurrentSubscription(tenant)?.accessUntil)}
                  />
                  <DetailItem
                    label="Valor de implantacao"
                    value={
                      getCurrentSubscription(tenant)?.contractedSetupFee
                        ? formatMoney(getCurrentSubscription(tenant)?.contractedSetupFee)
                        : '-'
                    }
                  />
                </div>
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:col-span-2">
                ...
                <p className="mt-1 whitespace-pre-wrap text-sm font-semibold text-slate-700">
                  {tenant.internalNotes || 'Sem observacoes.'}
                </p>
              </div>
            </div>
